...
const getSystemStatus = async (system, response) => {
    try {
        ...
        if (systemName.includes('vira') || systemName.includes('confluence')) {
            newBroadcast = await getBroadcastedMessages('https://confluence.volvocars.biz/')
            ...
            newBroadcast = await getBroadcastedMessages('https://gitlab.cm.volvocars.biz/api/v4/broadcast_messages')
